...
export async function POST(request: Request) {
  ...
  if (!name || !content) {
    return NextResponse.json({ error: 'Thiếu tên hoặc dữ liệu ảnh' }, { status: 400 })
  }

  const estimatedBytes = Math.ceil((content.length * 3) / 4)

  if (estimatedBytes > maxBytes) {
    return NextResponse.json({ error: 'Ảnh vượt quá 5 MB' }, { status: 400 })
  }

  const extension = type === 'image/jpeg' ? 'jpg' : type.split('/')[1]
  const baseName = safeFilename(name.replace(/\.[^.]+$/, '')) || 'image'
  const filename = `public/images/blog/${baseName}-${Date.now()}.${extension}`
  const token = process.env.GITHUB_TOKEN
  const repo = process.env.GITHUB_REPO

  if (!token || !repo) {
    return NextResponse.json({ error: 'Thiếu cấu hình GitHub' }, { status: 500 })
  }
  ...
  const path = `/${filename.replace(/^public\//, '')}`
